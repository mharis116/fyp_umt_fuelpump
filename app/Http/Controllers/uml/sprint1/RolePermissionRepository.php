...
class RolePermissionRepository
{
        ...
        if($controllerDetails['module_code'] && $module_permission_type_code){
            ...
            if(!$permission){
                if ($request->expectsJson()) {
                    ...
                    return redirect()->route('losted')->with('error', 'No permissions found for this module request');
                }
            ...
            if($permission){
                ...
                if ($request->expectsJson()) {
                    ...
                    return redirect()->route('losted')->with('error', 'Unauthorized access');
                }
            ...
            if ($request->expectsJson()) {
                ...
                return redirect()->route('losted')->with('error', 'No permissions defined for this module');
            }
    ...
    public function checkIfAnyRouteHasPermission(array $routeNames, User $user = null){
        foreach($routeNames as $routeName){
            if($this->getRoutePermission($routeName, $user)){
                return true;
            }
        }
        return false;
