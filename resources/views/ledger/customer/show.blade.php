...
<nav class="page-breadcrumb">
  <ol class="breadcrumb">
    <li class="breadcrumb-item"><a href="{{route('custledger.index')}}">Ledgers</a></li>
    <li class="breadcrumb-item"><a href="{{route('custledger.index')}}">Customer Ledgers</a></li>
            ...
            <tbody>
                @foreach ($sale as $s)
                    <tr class="tr pointer" id="{{$s->type}}">
                        ...
                        <td>{{$s->invoice_no}}</td>
                        <td>Rs.{{$s->retail_amount}}</td>
                        <td>{{$s->adjustment?$s->adjustment:0}} Rs</td>
                        <td>Rs.{{$s->retail_amount + $s->adjustment}}</td>
