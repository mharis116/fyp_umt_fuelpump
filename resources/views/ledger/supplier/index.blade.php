...
<nav class="page-breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active" aria-current="page"><a href="{{Request::get('test') == 1 ? route('report.credit'):route('supledger.index')}}">{{ Request::get('test') == 1 ? 'Credit Reports':'Suppliers' }}</a></li>
    @if (Request::get('test') == 1)
      <li class="breadcrumb-item active" aria-current="page"><a href="{{route('supledger.index','test=1')}}">Suppliers Credit Info</a></li>
    @endif
  </ol>
</nav>
<br>
  ...
  <div class="col-md-12 grid-margin stretch-card">
    <div class="card">
      <div class="card-body">
        ...
        <h6 class="card-title">{{Request::get('test') == 1 ? 'Suppliers Credit Info':'Suppliers Ledgers'}}</h6>
        <br>
          ...
          <table id="dataTableExample" class="table">
            ...
            <tbody>
              ...
              <tr class="tr pointer" id="{{$d->cust_name}}">
                <td>{{$d->cust_name}}</td>
                @if (Request::get('test') != 1)
                  <td><span data-toggle="tooltip" data-placement="bottom" id='tam{{$i}}' onmouseover="toWords({{$t}},'tam{{$i}}')" >Rs.{{$t?$t:0}}</span></td>
                  <td>{{$d->adj?$d->adj:0}} Rs</td>
                  <td><span data-toggle="tooltip" data-placement="bottom" id='tam{{$i}}' onmouseover="toWords({{$st}},'tam{{$i}}')" >Rs.{{$st?$st:0}}</span></td>
                ...
                @if (Request::get('test') != 1)
                  <td class="no-print">
                    <a href="{{route('supledger.show',$d->id)}}">
